...
- **`MonoCloudCodedException`** sits between `MonoCloudRequestException` and the statuses the apis report an `error_code` with — 400, 402, 403, 404, 409 — and is the only place `errorCode` is exposed. The rest (401, 422, 429, 500) extend `MonoCloudRequestException` directly and deliberately have no `errorCode`: nothing server-side sets one for them, so offering the member would only ever return undefined. Adding a status to the coded set means reparenting its exception here, not moving the getter up.
- **`MonoCloudClientBase.throwProblem`** (protected) is the seam for an sdk whose api reports errors this status mapping can't express — one carrying a machine-readable error code, say. Such an sdk overrides it, throws its own exception for the codes it declares, and calls `super.throwProblem` for everything else. Two constraints on what it throws: it must extend `MonoCloudException`, since `processRequest`'s catch replaces anything else with "Something went wrong."; and the status hierarchy stays here rather than being reimplemented per sdk, so an sdk-specific 404 should extend `MonoCloudNotFoundException` and keep that `instanceof` holding.
